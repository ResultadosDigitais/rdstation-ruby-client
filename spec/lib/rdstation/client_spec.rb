  ...
  describe "access_token" do
    context "when access_token is given" do
      ...
      it "instantiates it's services using this token" do
        expect(RDStation::Contacts).to receive(:new).with(token)
        expect(RDStation::Events).to receive(:new).with(token)
        expect(RDStation::Fields).to receive(:new).with(token)
        expect(RDStation::Webhooks).to receive(:new).with(token)
